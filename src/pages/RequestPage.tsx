  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: dup } = await supabase
        ...
        .limit(1)
      if (dup && dup.length > 0) setWarning(t.pages.request.duplicateWarning)
      ...
      if (err) throw err
      setSuccess(true)
      setName(''); setDate(''); setShiftType('evening'); setNotes('')
    } catch (e: unknown) {
    ...
    return (
      <div className="animate-slide-in max-w-md mx-auto pt-8">
        <div className="card p-10 text-center">
          ...
          <p className="text-ink-500 mb-6">{t.pages.request.successDesc}</p>
          <button onClick={() => setSuccess(false)} className="btn-secondary w-full">הגש בקשה נוספת</button>
        </div>
      </div>
    )
  }
      ...
      <div className="card p-6">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div>
            ...
            <textarea className="input resize-none" rows={3} placeholder={t.form.notesPlaceholder}
